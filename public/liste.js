    ...
    fetch('/session').then((userdata) => {
        ...
        if (dataUser.userid == undefined || dataUser.userid == null) {
            ...
            async function refreshParties() {
                ...
                await fetch("/recupPartie").then(reponse => reponse.json())
                    .then(async data => {
                        data.forEach(async game => {
                            ...
                            if (document.getElementById(game._id)) {
                                ...
                                if (game.createurPartie == session.userid) {
                                    item.innerHTML = " Nom de la partie: " + game.nomPartie + "<br> Joueur 1: " + game.J1.nomJ1 + "<br> Joueur 2 : " + game.J2.nomJ2 + "<br> Etat : " + game.statue + "<br><form action='/deletePartie' method='get'><button class='w3-button' type='submit'" + game._id + "'>Supprimer</button></form> "
                                        + "<br> <form action='/partieEncour/" + game._id + "' method='get'><button class='w3-button' type='submit'" + game._id + "'>Rejoindre</button></form>";
                                } else {
                                ...
                                li.id = game._id
                                if (game.createurPartie == session.userid) {
                                    li.innerHTML = " Nom de la partie: " + game.nomPartie + "<br> Joueur 1: " + game.J1.nomJ1 + "<br> Joueur 2 : " + game.J2.nomJ2 + "<br> Etat : " + game.statue + "<br><form action='/deletePartie' method='get'><button class='w3-button' type='submit'" + game._id + "'>Supprimer</button></form> "
                                        + "<br> <form action='/partieEncour/" + game._id + "' method='get'><button class='w3-button' type='submit'" + game._id + "'>Rejoindre</button></form>";
